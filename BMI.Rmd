...
tse %>%
  tse_meta(rownames = FALSE) %>% 
  lm(scale(BMI) ~ scale(shannon) + BL_AGE + MEN + PREVAL_DIAB + PREVAL_HFAIL_STRICT + CURR_SMOKE +
...
tse %>%
  tse_meta(rownames = FALSE) %>% 
  lm(scale(VYOTARO) ~ scale(shannon) + BL_AGE + MEN + PREVAL_DIAB + PREVAL_HFAIL_STRICT + CURR_SMOKE +
                                    ALKI2_FR02 + HFC + Q57, data = .) %>% 
  broom::tidy(conf.int = TRUE) %>%
  round_numeric_columns() %>% 
  DT::datatable(caption = "Alpha diversity to Vyot")
...
tse %>%
  tse_meta(rownames = FALSE) %>% 
  lm(scale(WHR) ~ scale(shannon) + BL_AGE + MEN + PREVAL_DIAB + PREVAL_HFAIL_STRICT + CURR_SMOKE +
                                    ALKI2_FR02 + HFC + Q57, data = .) %>% 
  broom::tidy(conf.int = TRUE) %>%
  round_numeric_columns() %>% 
  DT::datatable(caption = "Alpha diversity to WHR")
